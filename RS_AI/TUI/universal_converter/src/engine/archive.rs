...
impl Compressor for ArchiveEngine {
    fn is_password_protected(&self, archive: &Path) -> bool {
        ...
        if filename.ends_with(".zip") {
            ...
            {
                for i in 0..zip.len() {
                    if let Err(zip::result::ZipError::UnsupportedArchive(zip::result::ZipError::PASSWORD_REQUIRED)) =
                        zip.by_index(i)
                    {
                        return true;
                    }
            ...
            {
                let file_mut = file;
                if let Err(sevenz_rust::Error::PasswordRequired) =
                    sevenz_rust::SevenZReader::new(file_mut, len, sevenz_rust::Password::empty())
                {
                    return true;
                }
            }
    ...
    fn extract_archive(
        ...
        if is_double_archive {
            ...
            if let Some(stdout) = child.stdout.take() {
                ...
                for l in io::BufRead::lines(reader).map_while(Result::ok) {
                    let trimmed = l.trim();
    ...
    fn list_archive_contents(&self, archive: &Path) -> anyhow::Result<Vec<String>> {
        ...
        // CLI fallback
        let output = std::process::Command::new("7z").arg("l").arg(archive).output()?;
