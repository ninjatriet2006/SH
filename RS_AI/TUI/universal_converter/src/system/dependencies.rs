...
pub fn prompt_install(missing: &[String]) -> anyhow::Result<()> {
        ...
        if answer.is_empty() || answer == "y" {
            ...
            if !pkgs.is_empty() {
                ...
                if run_as_admin(&cmd).is_ok() {
                    ...
                    println!(
                        "[❌] Cài đặt thất bại. Vui lòng cài thủ công bằng lệnh: sudo apt install {}",
                        pkg_list
                    );
                }
            }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        println!(
            "- libreoffice: Tải từ libreoffice.org hoặc qua winget: `winget install TheDocumentFoundation.LibreOffice`"
        );
