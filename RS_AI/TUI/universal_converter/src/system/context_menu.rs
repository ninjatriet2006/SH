...
pub fn install_nemo_action(mode: &str) -> anyhow::Result<()> {
    #[cfg(target_os = "linux")]
    {
        ...
        let nemo_check = std::process::Command::new("which").arg("nemo").output();
...
"#,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let (key_name, display_name) = match mode {
            "default" => (
                "universal_converter_default",
                "Chuyển đổi với Universal Converter (Default)",
            ),
            "config" => (
                "universal_converter_config",
                "Chuyển đổi với Universal Converter (Config)",
            ),
        ...
        run_as_admin(&cmd)?;
        println!(
            "[✅] Đã cài đặt Context Menu ({}) trên Windows Explorer thành công!",
            mode
        );
    }
