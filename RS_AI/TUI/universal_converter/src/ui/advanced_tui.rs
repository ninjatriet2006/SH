...
fn run_app<B: ratatui::backend::Backend>(terminal: &mut Terminal<B>, app: &mut TuiApp) -> io::Result<()> {
...
fn draw_ui(f: &mut ratatui::Frame, app: &mut TuiApp) {
    ...
    let tabs = Tabs::new(titles)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(" UNIVERSAL CONVERTER - DASHBOARD NÂNG CAO "),
        )
    ...
    let footer =
        Paragraph::new(Line::from(footer_text)).block(Block::default().borders(Borders::ALL).title(" Trợ giúp nhanh "));
...
fn draw_files_tab(f: &mut ratatui::Frame, area: Rect, app: &mut TuiApp) {
    ...
    let list = List::new(items)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(" Danh sách File được chọn "),
        )
    ...
    let detail_block =
        Paragraph::new(detail_text).block(Block::default().borders(Borders::ALL).title(" Chi tiết tệp tin "));
...
fn draw_config_tab(f: &mut ratatui::Frame, area: Rect, app: &mut TuiApp) {
    ...
    let config_panel = Paragraph::new(config_lines).block(
        Block::default()
            .borders(Borders::ALL)
            .title(" Cấu hình hiện tại (Nạp từ YAML) "),
    );
...
fn draw_monitor_tab(f: &mut ratatui::Frame, area: Rect, app: &mut TuiApp) {
    ...
    let cpu_gauge = Gauge::default()
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(" Tổng hiệu năng sử dụng CPU "),
        )
        .gauge_style(
            Style::default()
                .fg(Color::Green)
