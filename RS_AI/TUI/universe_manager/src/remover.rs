            ...
            for line in content.lines() {
                if line.contains(&desktop_filename) {
                    if let Some(eq_idx) = line.find('=') {
                        ...
                        let cleaned_apps: Vec<&str> = apps_list
                            .split(';')
...
pub fn find_orphaned_leftovers() -> Vec<PathBuf> {
                    ...
                    && meta.is_dir()
                {
                    ...
                    if !is_matched {
                        for active_name in &active_names {
                            if folder_lower == *active_name
                                || (active_name.len() > 3 && folder_lower.contains(active_name))
                            {
                                is_matched = true;
                                break;
                            }
    ...
    #[cfg(windows)]
    {
        ...
        let output = Command::new("reg").args(&["query", registry_base]).output();
            ...
            for line in stdout.lines() {
                if line.starts_with(registry_base) {
                    if let Some(key_name) = line.split('\\').last() {
                        ...
                        if !is_matched {
                            for active_name in &active_names {
                                if key_lower == *active_name
                                    || (active_name.len() > 3 && key_lower.contains(active_name))
                                {
                                    is_matched = true;
                                    break;
                                }
...
pub fn uninstall_local_portable(entry: &crate::config::AppEntry) -> Result<(), String> {
    ...
    #[cfg(windows)]
    {
        ...
        for key in reg_keys {
            let check = Command::new("reg").args(&["query", &key]).output();
            if let Ok(out) = check {
    ...
    if install_path == downloads
        || install_path == documents
        || install_path == desktop
        || install_path == pictures
        || install_path == music
        || install_path == videos
    {
        return Err(format!(
    ...
    match install_type {
        InstallType::Moved => {
            if install_path.exists()
                && install_path.is_dir()
                && let Some(filename) = install_path.file_name()
            {
                if filename.to_string_lossy() == *app_id {
                    ...
                    fs::remove_dir_all(install_path).map_err(|e| format!("Lỗi khi xoá thư mục ứng dụng: {}", e))?;
                } else {
                    return Err(format!(
        ...
        InstallType::InPlace => {
            ...
            if let Some(parent_path) = exe_parent {
                ...
                {
                    println!(
                        "\nPhát hiện ứng dụng In-Place nằm trong thư mục riêng biệt: {:?}",
                        parent_path
                    );
                    let confirm_del = Confirm::new()
    ...
    if let Some(entry) = entry_opt {
        ...
        if let Some(entry) = system_apps.iter().find(|a| a.id == app_id) {
            match entry.package_type.as_deref() {
                ...
                _ => Err(format!(
                    "Kiểu ứng dụng không hỗ trợ gỡ cài đặt: {:?}",
                    entry.package_type
                )),
            }
        } else {
