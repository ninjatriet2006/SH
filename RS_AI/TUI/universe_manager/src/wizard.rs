...
fn integrate_single_app(
    ...
    // 4. Perform detection
    let detection = detector::detect(&final_source_path).map_err(|e| format!("Lỗi quét thông tin: {}", e))?;
    ...
    if detection.is_appimage {
        ...
        if !detection.desktop_templates.is_empty() {
            ...
            let template_paths: Vec<String> = detection
                .desktop_templates
                .iter()
            ...
            if selection < template_paths.len() {
                ...
                if let Some(meta_exec) = meta.exec {
                    ...
                    if let Some(found_exec) = detection
                        .executables
                        .iter()
                        .find(|e| e.file_name().unwrap_or_default().to_string_lossy() == bin_filename)
                    {
                        exec_rel_path = found_exec
                            .strip_prefix(&final_source_path)
        ...
        if exec_rel_path.as_os_str().is_empty() {
            if detection.executables.is_empty() {
                // Let user type it
                let typed_exec_opt = read_line_with_esc(
                    "Không tìm thấy file chạy tự động. Nhập tên file chạy chính (ví dụ: run.sh)",
                    None,
                )
                ...
                exec_rel_path = found.strip_prefix(&final_source_path).unwrap_or(found).to_path_buf();
                println!("Tự động chọn file chạy duy nhất tìm thấy: {:?}", exec_rel_path);
            } else {
                // Select from list
                let exec_options: Vec<String> = detection
                    .executables
                    .iter()
                    .map(|p| {
                        p.strip_prefix(&final_source_path)
                            .unwrap_or(p)
                            .to_string_lossy()
                            .to_string()
                    })
        ...
        if icon_path.is_none() && !detection.icons.is_empty() {
            let icon_options: Vec<String> = detection
                .icons
                .iter()
                .map(|p| {
                    p.strip_prefix(&final_source_path)
    ...
    if mode == 0 {
        ...
        if let Some(existing) = local_apps.iter().find(|a| a.id == temp_app_id) {
            ...
            println!(
                "\nPhát hiện ứng dụng trùng khớp trong hệ thống: {} (ID: {})",
            ...
            match choice_opt {
                ...
                Some(1) => {
                    ...
                    if local_apps.is_empty() {
                        ...
                        let app_selections: Vec<String> = local_apps
                            .iter()
                        ...
                        if let Some(sel) = selection_opt {
                            ...
                            default_symlink_name = existing
                                ...
                                .map(|f| f.to_string_lossy().to_string());
                            default_terminal =
                                parse_terminal_from_desktop(&existing.desktop_file).or(existing.is_custom);
                            default_icon_path = existing.icon_path.clone();
                            update_target_id = Some(existing.id.clone());
                            is_update = true;
                        } else {
        ...
        let app_name_opt = read_line_with_esc(
            "Nhập tên hiển thị của ứng dụng",
            Some(default_app_name.as_deref().unwrap_or(&name)),
        )
        .map_err(|e| format!("Lỗi nhập tên: {}", e))?;
        app_name = match app_name_opt {
            Some(val) if !val.trim().is_empty() => val,
        ...
        if detected_app_id != target_id {
            // Warning mismatch
            println!(
                "\nCẢNH BÁO: Tên ứng dụng mới quét được ({}) khác với ứng dụng bạn đã chọn cập nhật ({}).",
                name,
                default_app_name.as_deref().unwrap_or("")
            );
            println!("  [Phím tắt: Enter để chọn mặc định | Esc để huỷ/thoát]");
            let confirm_opt = Confirm::new()
    ...
    let install_type = if install_selection == 0 {
        InstallType::Moved
    } else {
        InstallType::InPlace
    };
    ...
    let create_symlink = match create_symlink_opt {
        Some(val) => val,
        None => return Ok(false),
    };

    if create_symlink {
        let default_cmd = default_symlink_name.unwrap_or_else(|| app_name.to_lowercase().replace(' ', "-"));
    ...
    if is_update {
        ...
        if let Some(existing) = all_scanned.iter().find(|a| a.id == old_id) {
            ...
            if let Some(parent) = path.parent()
                && parent.exists()
                && is_portable_path(&existing.exec_path)
            {
    ...
    if !new_icon_exists && let Some(ref old_icon_str) = default_icon_path {
        let old_icon_path = Path::new(old_icon_str);
        if old_icon_path.exists() {
            let file_name = old_icon_path.file_name().unwrap_or_default();
            let temp_icon = std::env::temp_dir().join(format!("um_icon_{}_{}", app_id, file_name.to_string_lossy()));
    ...
    if final_icon.is_none() {
        let registry_match = ONLINE_ICONS.iter().find(|item| item.app_ids.contains(&app_id.as_str()));

        let mut downloaded = false;
        if let Some(online_item) = registry_match {
            println!(
                "\nKhông tìm thấy file icon cục bộ. Tìm thấy link tải icon trực tuyến cho ứng dụng: {}",
                app_name
            );
            println!("  [Phím tắt: Enter để chọn mặc định | Esc để huỷ/thoát]");
        ...
        if !downloaded {
            ...
            match use_sys_icon_opt {
                ...
                Some(false) => {
                    // Let user type it
                    let typed_icon_opt = read_line_with_esc(
                        "Nhập đường dẫn file icon hoặc tên icon hệ thống (để trống nếu không dùng)",
                        None,
                    )
                    .map_err(|e| format!("Lỗi nhập: {}", e))?;
                    if let Some(typed_icon) = typed_icon_opt
                        && !typed_icon.trim().is_empty()
                    {
    ...
    if delete_old_dir
        && let Some(ref old_dir) = old_app_dir
        && old_dir.exists()
        && old_dir != &final_source_path
    {
...
pub fn run_wizard(initial_path: Option<String>) -> Result<bool, String> {
    ...
    let local_apps: Vec<_> = all_scanned_apps
        .into_iter()
        .filter(|a| a.package_type.as_deref().unwrap_or("Local") == "Local")
        .collect();
        ...
        match choice_opt {
            Some(0) => {
                ...
                loop {
                    ...
                    let path_opt =
                        read_line_with_esc(&prompt_msg, None).map_err(|e| format!("Lỗi nhập liệu: {}", e))?;
                    match path_opt {
                        Some(p) => {
            ...
            Some(1) => {
                ...
                if discovered.is_empty() {
                    ...
                    let selections: Vec<String> = discovered.iter().map(|p| p.to_string_lossy().to_string()).collect();
