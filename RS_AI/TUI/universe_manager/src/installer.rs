...
pub fn search_apps(query: &str) -> Vec<SearchResult> {
    ...
    if Command::new("choco").arg("--version").output().is_ok()
        && let Ok(out) = Command::new("choco").args(["search", query, "--limit-output"]).output()
    {
        let combined = String::from_utf8_lossy(&out.stdout).to_string();
        for line in combined.lines() {
...
pub fn install_app(id: &str, source: &str) -> Result<String, String> {
    let status = match source {
        "winget" | "msstore" => Command::new("winget")
            ...
            .status(),
        "chocolatey" => Command::new("choco").args(["install", id, "-y"]).status(),
        "scoop" => Command::new("powershell")
            .args(["-NoProfile", "-Command", &format!("scoop install {}", id)])
            .status(),
