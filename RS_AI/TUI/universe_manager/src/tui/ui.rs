...
pub fn draw(f: &mut Frame, app: &mut App) {
    if app.needs_initial_scan {
        ...
        let loading_block = Block::default()
            .borders(Borders::ALL)
            .title("Universe Manager - Loading");
    ...
    let mut header_text_spans = vec![Span::styled(
        "Hệ thống quản lý ứng dụng đa năng ",
        Style::default().fg(Color::White),
    )];

    if app.current_screen == Screen::UpdateManager {
        header_text_spans.push(Span::styled(
            format!("(Tổng số bản cập nhật: {})", app.update_entries.len()),
            Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD),
        ));
    } else {
        header_text_spans.push(Span::styled(
            format!("(Tổng số app: {})", app.apps_with_status.len()),
            Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD),
        ));
    }
    ...
    let footer_text = match app.current_screen {
        ...
        Screen::AppList => Line::from(vec![
            ...
            Span::styled(" | ", Style::default().fg(Color::DarkGray)),
            Span::styled(
                "Insert: Tiến vào màn hình Thao Tác",
                Style::default().fg(Color::LightGreen),
            ),
            Span::styled(" | ", Style::default().fg(Color::DarkGray)),
            Span::styled(
                "Backspace/Esc: Quay lại Menu chính",
        ...
        Screen::AppOperations => Line::from(vec![
            ...
            Span::styled(" | ", Style::default().fg(Color::DarkGray)),
            Span::styled(
                "Space: Chọn hành động để thực thi [x]",
                Style::default().fg(Color::Yellow),
            ),
            Span::styled(" | ", Style::default().fg(Color::DarkGray)),
            Span::styled(
                "Insert/Enter: Thực thi tất cả các hành động đã chọn",
                Style::default().fg(Color::LightGreen),
            ),
            Span::styled(" | ", Style::default().fg(Color::DarkGray)),
            Span::styled(
                "Backspace/Esc: Quay lại danh sách app",
                Style::default().fg(Color::LightRed),
            ),
        ]),
        Screen::UninstallList => Line::from(vec![
            ...
            Span::styled(" | ", Style::default().fg(Color::DarkGray)),
            Span::styled(
                "Space: Tích chọn ứng dụng cần gỡ [x]",
                Style::default().fg(Color::Yellow),
            ),
            Span::styled(" | ", Style::default().fg(Color::DarkGray)),
            Span::styled(
                "Insert/Enter: Tiến hành gỡ cài đặt hoàn toàn",
                Style::default().fg(Color::LightGreen),
            ),
            Span::styled(" | ", Style::default().fg(Color::DarkGray)),
            Span::styled(
        ...
        Screen::AutostartManager => Line::from(vec![
            ...
            Span::styled(" | ", Style::default().fg(Color::DarkGray)),
            Span::styled(
                "Enter/Delete: Gỡ bỏ chương trình khỏi Startup",
        ...
        let rows: Vec<Row> = app
            ...
            .map(|(i, &app_idx)| {
                ...
                let source_style = if i == app.selected_index {
                    match source_str {
                        ...
                        "Snap" => Style::default()
                            ...
                            .add_modifier(Modifier::BOLD),
                        "Homebrew" => Style::default()
                            .bg(Color::Blue)
                            .fg(Color::Cyan)
                            .add_modifier(Modifier::BOLD),
                        "Winget" => Style::default()
                            .bg(Color::Blue)
                            .fg(Color::LightGreen)
                            .add_modifier(Modifier::BOLD),
                        "Scoop" => Style::default()
                ...
                Row::new(vec![
                    ...
                    Cell::from(run_status).style(if i == app.selected_index {
                        run_style.bg(Color::Blue).add_modifier(Modifier::BOLD)
                    } else {
                        run_style
                    }),
                ])
    ...
    if let Some((entry, status)) = app.apps_with_status.get(app.selected_index) {
        ...
        details_text.push(Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default()
                    .fg(if is_running { Color::Green } else { Color::Red })
                    .add_modifier(Modifier::BOLD),
            ),
        ]));
        ...
        match status {
            AppStatus::Healthy => {
                details_text.push(Line::from(vec![Span::styled(
                    "  [OK] Xanh lá - launcher và đường dẫn command line liên kết tốt.",
                    Style::default().fg(Color::Green).add_modifier(Modifier::BOLD),
                )]));
            }
            AppStatus::Degraded(issues) => {
                details_text.push(Line::from(vec![Span::styled(
                    "  [WARNING] Vàng - Lỗi nhẹ hoặc thiếu shortcut:",
                    Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD),
                )]));
                for issue in issues {
                    details_text.push(Line::from(vec![
            ...
            AppStatus::Broken(issues) => {
                details_text.push(Line::from(vec![Span::styled(
                    "  [CRITICAL] Đỏ - Thiếu file chạy gốc hoặc thư mục cài đặt:",
                    Style::default().fg(Color::Red).add_modifier(Modifier::BOLD),
                )]));
                for issue in issues {
                    details_text.push(Line::from(vec![
...
fn draw_app_operations(f: &mut Frame, app: &App, area: Rect) {
    ...
    let list = List::new(items).block(action_block);
    ...
    let mut selected_apps_text = vec![
        Line::from(vec![Span::styled(
            " Danh sách ứng dụng sẽ nhận lệnh:",
    ...
    selected_apps_text.push(Line::from("---------------------------------------------"));
    selected_apps_text.push(Line::from(vec![Span::styled(
        " Nhấn [Space] để tick chọn Hành động cần chạy.",
        Style::default().fg(Color::Yellow),
    )]));
    selected_apps_text.push(Line::from(vec![Span::styled(
        " Nhấn [Insert] hoặc [Enter] để thực thi ngay.",
        Style::default().fg(Color::Green).add_modifier(Modifier::BOLD),
    )]));
    selected_apps_text.push(Line::from(vec![Span::styled(
        " Nhấn [Backspace] để quay lại.",
        Style::default().fg(Color::LightRed),
    )]));
...
fn draw_uninstall_list(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let mut warn_text = vec![
        Line::from(vec![Span::styled(
            " CẢNH BÁO NGUY HIỂM / ĐỒNG BỘ XOÁ FILE",
            Style::default().fg(Color::Red).add_modifier(Modifier::BOLD),
        )]),
        Line::from(""),
        ...
        Line::from(vec![
            ...
            Span::styled(
                "Xoá toàn bộ launcher (.desktop/.lnk) ra khỏi hệ thống.",
                Style::default().fg(Color::White),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                "Xoá các liên kết CLI command symlink tại ~/.local/bin/.",
                Style::default().fg(Color::White),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                "Đối với ứng dụng kiểu Moved: Xoá sạch thư mục gốc chứa app.",
                Style::default().fg(Color::White),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                "Đối với ứng dụng kiểu In-Place: Chỉ xoá tệp nhị phân chính và icon, KHÔNG xoá thư mục trừ phi ứng dụng nằm ở thư mục riêng biệt.",
                Style::default().fg(Color::White),
            ),
        ]),
        Line::from(""),
        ...
        Line::from(""),
        Line::from(vec![Span::styled(
            " Các ứng dụng được chọn để gỡ cài đặt:",
            Style::default().fg(Color::White).add_modifier(Modifier::BOLD),
        )]),
    ];
    ...
    warn_text.push(Line::from(vec![Span::styled(
        " Nhấn [Space] bên trái để tích chọn ứng dụng.",
        Style::default().fg(Color::Yellow),
    )]));
    warn_text.push(Line::from(vec![Span::styled(
        " Nhấn [Insert] hoặc [Enter] để thực thi gỡ cài đặt hoàn toàn.",
        Style::default().fg(Color::Red).add_modifier(Modifier::BOLD),
    )]));
    warn_text.push(Line::from(vec![Span::styled(
        " Nhấn [Backspace] để quay lại.",
    ...
    let paragraph = Paragraph::new(warn_text).block(warn_block).wrap(Wrap { trim: true });
    f.render_widget(paragraph, main_chunks[1]);
}
        ...
        let rows: Vec<Row> = app
            ...
            .map(|(i, entry)| {
                ...
                Row::new(vec![
                    ...
                    Cell::from(status_str).style(if i == app.autostart_index {
                        status_style.bg(Color::Blue).add_modifier(Modifier::BOLD)
                    } else {
                        status_style
                    }),
                ])
...
fn draw_update_manager(f: &mut Frame, app: &mut App, area: Rect) {
    if app.needs_update_scan {
        let loading_block = Block::default()
            .borders(Borders::ALL)
            .title("Cập nhật phần mềm/hệ thống");
        ...
        if app.update_entries.is_empty() {
            ...
            let items: Vec<Row> = app
                ...
                .enumerate()
                .map(|(i, entry)| {
                    let checked = if app.checked_updates.contains(&i) {
                        " [x]"
                    } else {
                        " [ ]"
                    };
                    let check_style = if app.checked_updates.contains(&i) {
                        Style::default().fg(Color::Yellow)
                    } else {
                        Style::default().fg(Color::DarkGray)
                    };
                    ...
                    Row::new(vec![
                        ...
                        Cell::from(entry.available_version.clone()).style(if i == app.update_index {
                            Style::default()
                                .bg(Color::Blue)
                                .fg(Color::Green)
                                .add_modifier(Modifier::BOLD)
                        ...
                        Cell::from(entry.source.clone()).style(if i == app.update_index {
                            Style::default()
                                .bg(Color::Blue)
                                .fg(Color::Yellow)
                                .add_modifier(Modifier::BOLD)
            ...
            let table = Table::new(items, widths).header(header).block(block).column_spacing(1);
...
fn draw_app_installer(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let search_block = Block::default()
        .borders(Borders::ALL)
        .border_style(if app.is_install_searching {
            Style::default().fg(Color::Yellow)
        } else {
            Style::default().fg(Color::DarkGray)
        })
    ...
    let search_text =
        Paragraph::new(app.install_search_query.clone() + if app.is_install_searching { "█" } else { "" })
            .block(search_block)
            .style(Style::default().fg(if app.is_install_searching {
                Color::White
            } else {
                Color::Gray
            }));
