...
pub fn enable_autostart(app: &AppEntry) -> Result<(), String> {
    ...
    if !autostart_dir.exists() {
        fs::create_dir_all(&autostart_dir).map_err(|e| format!("Lỗi tạo thư mục autostart: {}", e))?;
    }
    ...
    if src.exists() {
        fs::copy(src, &dest).map_err(|e| format!("Lỗi copy file cấu hình vào autostart: {}", e))?;
    } else {
        ...
        let content = format!(
            ...
            Terminal=false\n\
            Categories={}\n",
            app.name, comment, app.exec_path, app.install_path, icon_line, categories
        );
        fs::write(&dest, content).map_err(|e| format!("Lỗi ghi file autostart: {}", e))?;
    }
    Ok(())
}
...
pub fn disable_autostart(app: &AppEntry) -> Result<(), String> {
    ...
    if autostart_file.exists() {
        fs::remove_file(autostart_file).map_err(|e| format!("Lỗi xoá tệp autostart: {}", e))?;
    }
    Ok(())
}
                ...
                for file in files.flatten() {
                        ...
                        && let Ok(content) = fs::read_to_string(&path)
                    {
                        ...
                        for line in content.lines() {
                            ...
                            } else if let Some(e) = trimmed.strip_prefix("Exec=") {
                                exec = Some(e.to_string());
    ...
    #[cfg(windows)]
    {
        if entry.location.contains("Registry (HKCU)") {
            ...
            if status.success() {
                ...
                Err("Không thể xoá registry key".to_string())
            }
            ...
            if status.success() {
                ...
                Err("Không thể xoá registry key (yêu cầu quyền Admin)".to_string())
            }
