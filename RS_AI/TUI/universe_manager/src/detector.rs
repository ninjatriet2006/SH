...
impl DesktopMetadata {
    pub fn parse_file<P: AsRef<Path>>(path: P) -> Self {
        ...
        for line in content.lines() {
            ...
            if in_desktop_entry && let Some(pos) = trimmed.find('=') {
...
pub fn detect<P: AsRef<Path>>(target: P) -> Result<DetectionResult, std::io::Error> {
    ...
    for entry in WalkDir::new(&path).max_depth(4).into_iter().filter_map(Result::ok) {
        ...
        if let Some(ext) = &extension
            && (ext == "png" || ext == "svg" || ext == "jpg" || ext == "jpeg")
        {
...
pub fn scan_for_unintegrated_apps(config: &crate::config::Config) -> Vec<PathBuf> {
    ...
    for app in &config.apps {
        if let Some(ref sp) = app.source_path
            && let Ok(canon) = Path::new(sp).canonicalize()
        {
        ...
        for entry in WalkDir::new(&dir).max_depth(2).into_iter().filter_map(Result::ok) {
            ...
            #[cfg(unix)]
            {
                ...
                if filename.ends_with(".appimage")
                    ...
                    && !registered_paths.contains(&canon)
                {
...
pub fn detect_framework(dir: &Path) -> String {
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        for entry in entries.flatten() {
            ...
            if name == "resources" && path.is_dir() && path.join("app.asar").exists() {
                has_asar = true;
            }
    ...
    if has_asar || (has_package_json && !has_flutter) {
        "Electron".to_string()
    } else if has_flutter {
        "Flutter".to_string()
    } else if has_qt {
