...
pub fn scan_all_system_apps() -> Vec<AppEntry> {
    ...
    if managed_dir.exists()
        && managed_dir.is_dir()
        && let Ok(entries) = fs::read_dir(managed_dir)
    {
    ...
    #[cfg(unix)]
    {
        if Command::new("brew").arg("--version").status().is_ok()
            && let Ok(out) = Command::new("brew").args(["list", "--formula"]).output()
        {
                ...
                for line in stdout.lines() {
                    ...
                    if in_apps && !trimmed.is_empty() {
                        ...
                        if !parts.is_empty() {
                            ...
                            let name_formatted =
                                app_name.chars().next().unwrap().to_uppercase().collect::<String>() + &app_name[1..];
                            let name_lower = name_formatted.to_lowercase();
