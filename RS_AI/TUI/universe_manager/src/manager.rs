    ...
    for base in &uninstall_bases {
        if let Ok(out) = Command::new("reg").args(&["query", base]).output() {
            ...
            for subkey in stdout.lines() {
                ...
                let clean_id = get_real_id(&app.id)
                    .to_lowercase()
                                                    ...
                                                    if meta.is_file() {
                                                            ...
                                                            .map_or(false, |e| e.to_ascii_lowercase() == "exe")
                                                        {
                                                            let filename = path
                                                                ...
                                                                .to_string_lossy()
                                                                .to_lowercase();
                                                            if filename.contains(&clean_name)
                                                                || clean_name.contains(&filename.replace(".exe", ""))
                                                            {
    ...
    pub fn is_running(&self, app: &AppEntry) -> bool {
                ...
                if is_custom {
                    ...
                    if let Some(ref start) = app.start_cmd {
                        ...
                        if !first_word.is_empty()
                            && cmd_str.contains(first_word)
                            && start.contains("flatpak")
                            && start.contains(real_id)
                        {
                            return true;
                        }
                    }
        ...
        for name in &self.names {
            ...
            for word in app.name.split_whitespace() {
                ...
                if clean_word.len() > 3
                    && clean_name.contains(&clean_word)
                    && clean_word != "client"
                    && clean_word != "player"
                    && clean_word != "manager"
                    && clean_word != "free"
...
pub fn start_app(app: &AppEntry) -> Result<(), String> {
    ...
    #[cfg(unix)]
    {
        if app.is_custom.unwrap_or(false)
            && let Some(ref start_cmd) = app.start_cmd
        {
            Command::new("sh")
                .arg("-c")
                .arg(start_cmd)
    ...
    #[cfg(windows)]
    {
        ...
        match status {
            ...
            _ => Command::new(&fallback_exe)
                ...
                .map_err(|e| {
                    format!(
                        "Không tìm thấy file chạy và chạy thử lệnh cmd/direct đều thất bại: {}",
                        e
                    )
                }),
        }
...
pub fn stop_app(app: &AppEntry) -> Result<(), String> {
    ...
    {
        if app.is_custom.unwrap_or(false)
            && let Some(ref stop_cmd) = app.stop_cmd
        {
    ...
    #[cfg(windows)]
    {
        ...
        for name in &snapshot.names {
            ...
            if matches {
                ...
                let _ = Command::new("taskkill").args(&["/F", "/IM", &target_exe]).status();
                stopped_any = true;
            }
        }
