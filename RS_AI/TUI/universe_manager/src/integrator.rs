...
use crate::config::{AppEntry, Config, InstallType};
...
pub fn integrate(params: IntegrationParams) -> Result<AppEntry, String> {
    ...
    // Clean up existing app of same ID if any
    {
        ...
        if let Some(existing) = config.apps.iter().find(|a| a.id == app_id) {
            println!(
                "[Update] Phát hiện ứng dụng '{}' đã tồn tại. Đang làm sạch phiên bản cũ...",
                app_id
            );
    ...
    let comment = params
        .comment
        .unwrap_or_else(|| format!("Được tích hợp bởi Universe Manager: {}", params.name));
    ...
    if params.create_symlink {
        ...
        if let Err(e) = symlink(&final_exec_path, &symlink_path) {
            println!(
                "Cảnh báo: Không thể tạo link command-line tại {:?}: {}",
                symlink_path, e
            );
        } else {
    ...
    config
        .save()
        .map_err(|e| format!("Không thể lưu cấu hình config.json: {}", e))?;
