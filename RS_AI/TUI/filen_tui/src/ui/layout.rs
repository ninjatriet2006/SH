...
use crate::app::{App, PaneState, PopupState, Screen};
...
fn draw_header(app: &App, f: &mut Frame, area: Rect) {
    ...
    let header_text = vec![Line::from(vec![
        ...
        Span::raw(" | "),
        Span::styled(
            format!("Tài khoản hoạt động: {}", active_email),
...
fn operations_whoami_cache(app: &App) -> String {
    app.active_account
        .clone()
        .unwrap_or_else(|| "Chưa đăng nhập".to_string())
}
...
fn draw_main_menu(app: &App, f: &mut Frame, area: Rect) {
    ...
    let active_server_webdav = if app.webdav_server.running {
        "🟢 Đang chạy"
    } else {
        "🔴 Đang tắt"
    };
    let active_server_s3 = if app.s3_server.running {
        "🟢 Đang chạy"
    } else {
        "🔴 Đang tắt"
    };
    ...
    let info_text = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                active_server_webdav,
                Style::default().fg(if app.webdav_server.running {
                    Color::LightGreen
                } else {
                    Color::Red
                }),
        ...
        Line::from(vec![
            ...
            Span::styled(
                active_server_s3,
                Style::default().fg(if app.s3_server.running {
                    Color::LightGreen
                } else {
                    Color::Red
                }),
        ...
        Line::from(" Thư mục cục bộ mặc định: ~/".to_string()),
    ];
...
fn draw_explorer(app: &mut App, f: &mut Frame, area: Rect) {
    ...
    let console_text = if !app.clipboard.is_empty() {
        let action = if app.clipboard_is_cut {
            "Cắt (Cut)"
        } else {
            "Sao chép (Copy)"
        };
        format!(
            "📋 Đang giữ {} mục trong Clipboard để {}. Bấm Ctrl+V ở pane đích để dán.",
            app.clipboard.len(),
            action
        )
    } else {
        let active_pane = if app.active_pane_left {
            &app.left_pane
        } else {
            &app.right_pane
        };
...
fn draw_pane(f: &mut Frame, pane: &mut PaneState, area: Rect, is_active: bool) {
    ...
    let block = Block::default()
        .title(Span::styled(
            pane_title,
            Style::default().fg(border_color).add_modifier(border_style),
        ))
        .borders(Borders::ALL)
...
fn draw_account(app: &App, f: &mut Frame, area: Rect) {
    ...
    let info_text = vec![
        ...
        Line::from(format!(
            " 💾 Dung lượng:    {} đã dùng của {}",
            app.storage_used, app.storage_max
        )),
        Line::from(vec![
...
fn draw_servers(app: &App, f: &mut Frame, area: Rect) {
    ...
    // 1. Draw Server Tabs
    let webdav_style = if app.active_server_tab == 0 {
        Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)
    } else {
        Style::default().fg(Color::DarkGray)
    };
    let s3_style = if app.active_server_tab == 1 {
        Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)
    } else {
        Style::default().fg(Color::DarkGray)
    };

    let tabs_text = vec![Line::from(vec![
    ...
    if app.active_server_tab == 0 {
        ...
        let https_str = if app.webdav_server.https {
            "Có (HTTPS)"
        } else {
            "Không (HTTP)"
        };
        fields.push(("HTTPS (Mã hóa): ", https_str));
        let action_str = if app.webdav_server.running {
            "▶ [ TẮT SERVER ]"
        } else {
            "▶ [ BẬT SERVER ]"
        };
        ...
        let https_str = if app.s3_server.https {
            "Có (HTTPS)"
        } else {
            "Không (HTTP)"
        };
        fields.push(("HTTPS (Mã hóa): ", https_str));
        let action_str = if app.s3_server.running {
            "▶ [ TẮT SERVER ]"
        } else {
            "▶ [ BẬT SERVER ]"
        };
    ...
    // 3. Draw Logs
    let logs = if app.active_server_tab == 0 {
        &app.webdav_server.logs
    } else {
        &app.s3_server.logs
    };
...
fn draw_footer(app: &App, f: &mut Frame, area: Rect) {
    let help_text = match app.current_screen {
        ...
        Screen::Explorer => {
            " Tab: Đổi bảng | Backspace: Lên thư mục | Alt+R: Đổi Pane Local/Cloud | Alt+N: Tạo thư mục | Alt+Y: Đổi tên | Alt+O: Menu Đặc biệt | Alt+T: Đồng bộ | Shift+V: Chọn vùng | Delete: Xóa | Esc: Về Menu chính "
        }
        Screen::Account => {
            " ▲/▼: Chọn Account | Alt+S: Đổi Hoạt động | Alt+N: Đăng nhập mới | Alt+D: Gỡ tài khoản | Alt+L: Đăng xuất | Alt+C/K: Xuất Config/API | Esc: Về Menu chính "
        }
...
fn draw_popups(app: &App, f: &mut Frame) {
    ...
    match &app.popup_state {
        ...
        PopupState::LoginInput {
            ...
            let pass_style = if *active_field == 1 {
                Style::default().fg(Color::Black).bg(Color::Cyan)
            } else {
                Style::default().fg(Color::White)
            };
            let keep_style = if *active_field == 2 {
                Style::default().fg(Color::Black).bg(Color::Cyan)
            } else {
                Style::default().fg(Color::White)
            };
            ...
            let email_val = if *active_field == 0 {
                format!(" {}█", email_buffer)
            } else {
                format!(" {} ", email_buffer)
            };
            let pass_val = if *active_field == 1 {
                format!(" {}█", masked_pass)
            } else {
                format!(" {} ", masked_pass)
            };
            let keep_val = if *active_field == 2 {
                format!(" {}█", keep_logged)
            } else {
                format!(" {} ", keep_logged)
            };
        ...
        PopupState::ViewFile { name, content, scroll } => {
            ...
            let footer = format!(
                " [▲/▼] Cuộn | [C] Sao chép | [Esc] Thoát | Dòng {} - {} / {}",
                scroll + 1,
                (scroll + height).min(content.len()),
                content.len()
            );
            let block = Block::default()
                .title(Span::styled(
    ...
    if app.is_loading {
        if !app.login_logs.is_empty() {
            ...
            let block = Block::default()
                .title(Span::styled(
                    " GIÁM SÁT TIẾN TRÌNH CLI (BACKEND LOGS) ",
                    Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD),
                ))
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Yellow));
