...
pub async fn handle_account_key(app: &mut App, key: KeyEvent) {
        ...
        match key.code {
            ...
            KeyCode::Char('c') | KeyCode::Char('C') => {
                if name.contains("API Key") {
                    ...
                    let config_lines: Vec<String> = content
                        .iter()
                        .filter(|line| {
                            !line.starts_with("✨") && !line.starts_with("⚠️") && !line.starts_with("Bấm [C]")
                        })
                        .cloned()
                        .collect();
    ...
    if let PopupState::TwoFAInput {
        email,
        password,
        keep_logged,
        mut twofa_buffer,
    } = app.popup_state.clone()
    {
        match key.code {
            KeyCode::Esc => {
                app.popup_state = PopupState::None;
        ...
        match key.code {
            ...
            KeyCode::Char('c') | KeyCode::Char('C') => {
                ...
                app.popup_state = PopupState::ViewFile {
                    ...
                    content: vec![
                        "Đang truy xuất cấu hình đăng nhập...".to_string(),
                        "Vui lòng đợi giây lát... ⏳".to_string(),
                    ],
                    scroll: 0,
                };
                let tx = app.msg_tx.clone();
            ...
            KeyCode::Char('k') | KeyCode::Char('K') => {
                ...
                app.popup_state = PopupState::ViewFile {
                    ...
                    content: vec![
                        "Đang truy xuất API Key cho Rclone...".to_string(),
                        "Vui lòng đợi giây lát... ⏳".to_string(),
                    ],
                    scroll: 0,
                };
