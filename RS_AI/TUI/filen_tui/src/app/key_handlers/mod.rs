...
use crate::app::{App, PopupState, Screen};
...
async fn handle_menu_key(app: &mut App, key: KeyEvent, quit: &mut bool) {
    match key.code {
        ...
        KeyCode::Enter => match app.main_menu_selected {
            ...
            if app.server_selected_field == 4 {
                ...
                let (title, buffer) = if app.active_server_tab == 0 {
                    ...
                    match app.server_selected_field {
                        ...
                        1 => (
                            "Nhập Secret Access Key S3".to_string(),
                            app.s3_server.secret_key.clone(),
                        ),
    ...
    if app.active_server_tab == 0 {
        ...
        if app.webdav_server.running {
            ...
            cmd.arg("webdav")
                ...
                .arg("--w-password")
                .arg(&app.webdav_server.pass)
                .arg("--w-port")
                .arg(&app.webdav_server.port);
            ...
            match cmd.spawn() {
                Ok(child) => {
                    ...
                    app.webdav_server
                        .logs
                        .push(format!("Đã khởi chạy WebDAV trên cổng {}.", app.webdav_server.port));
                }
                Err(e) => {
        ...
        if app.s3_server.running {
            ...
            cmd.arg("s3")
                ...
                .arg(&app.s3_server.secret_key)
                .arg("--s3-port")
                .arg(&app.s3_server.port);
            ...
            match cmd.spawn() {
                Ok(child) => {
                    ...
                    app.s3_server
                        .logs
                        .push(format!("Đã khởi chạy S3 trên cổng {}.", app.s3_server.port));
                }
                Err(e) => {
