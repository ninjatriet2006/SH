...
pub fn process_files(files: &[PathBuf], settings: &Settings) -> Vec<PathBuf> {
    ...
    let options = vec![
        ...
        "6. bmp",
        "7. tiff",
        "8. Bỏ qua (Giữ nguyên định dạng gốc)",
    ];
    let choice = Select::new("Lựa chọn định dạng đích:", options)
        .prompt()
        .unwrap_or("8. Bỏ qua");
    ...
    pb.set_style(
        ProgressStyle::default_bar()
            .template("   -> Tiến trình: [{bar:50.cyan/blue}] {percent}% ({pos}/{len} file) {msg}")
            .unwrap()
            .progress_chars("#>-"),
    );
    ...
    let processed_files: Vec<PathBuf> = files
        .par_iter()
        .filter_map(|file| {
                ...
                .output()
            {
                ...
                if let Ok(width) = out_str.trim().parse::<u32>()
                    && width < settings.min_upscale_width
                {
                    needs_upscale = true;
                }
            }
            ...
            if !needs_upscale && target_ext == "skip" {
                ...
                for _retry in 0..settings.max_retries {
                    ...
                    if needs_upscale {
                        cmd.arg("-vf")
                            .arg(format!("scale={}:-1", settings.target_upscale_width));
                    }
                    ...
                    if let Ok(status) = cmd.status()
                        && status.success()
                        && let Ok(metadata) = fs::metadata(&out_path)
                        && metadata.len() > 0
                    {
                        success = true;
                        break;
    ...
    final_files.sort_by(|a, b| crate::scanner::natural_sort(a.as_path(), b.as_path()));
