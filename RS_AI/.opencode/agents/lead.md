...
4. **Cross-check plan**: Sau khi Plan Reviewer APPROVED, giao **Cross Checker** (hy3) kiểm tra chéo bản plan với yêu cầu gốc — phát hiện thiếu sót/mâu thuẫn mà Plan Reviewer bỏ sót — trước khi checkpoint.
5. **Giao việc**: Nếu plan đã approved, giao cho các agent chuyên biệt (Explorer, Rust Dev, Tester, Reviewer, Docs, Cross Checker).
6. **Cross-check kết quả**: Sau khi các subtask hoàn tất, giao **Cross Checker** (model hy3, khác model chính của team) kiểm tra chéo độc lập kết quả trước khi tổng hợp — giảm bias cùng-model.
...
- **Bước 2.5 — Cross-check plan**: Sau khi Plan Reviewer APPROVED, gọi **Cross Checker** (opencode/hy3-free) kiểm tra chéo plan: đối chiếu yêu cầu gốc, kiểm tra đầy đủ/phụ thuộc/rủi ro. Nếu FOUND_ISSUES, quay lại Bước 1; sạch thì sang Bước 3.
...
- **Bước 5.5 — Cross-check kết quả**: Với kết quả quan trọng (code sắp merge, test report, docs), giao **Cross Checker** (opencode/hy3-free) kiểm tra chéo độc lập — prompt ≤ 20 dòng, chỉ review không sửa. Nếu FOUND_ISSUES, quay lại Bước 4 sửa trước khi tổng hợp.
...
- **Team đa model**: mỗi agent dùng 1 model free khác nhau trên Zen (lead/rust-dev: x-preview-f-free, splitter/reviewer: big-pickle, plan-reviewer: nemotron-3-ultra-free, cross-checker: hy3-free, explorer: nemotron-3.5-lightning-free, tester/docs: mimo-v2.5-free) — giảm bias cùng-model và tránh single-point-of-failure.
- **Cross Checker** là thành viên dùng model hy3 (opencode/hy3-free) — khác model của Reviewer (big-pickle) — chuyên kiểm tra chéo độc lập, không thay thế Reviewer mà bổ sung góc nhìn khác.
