...
use crate::app::{App, Screen, PopupState, AppEvent};
    ...
    match popup {
                    ...
                    if !new_name.is_empty() {
                        if old_name.starts_with("Nhập ") {
                            ...
                            if old_name.contains("WebDAV") {
                                if old_name.contains("Username") { app.webdav_server.user = new_name; }
                                else if old_name.contains("Password") { app.webdav_server.pass = new_name; }
                                else if old_name.contains("Port") { app.webdav_server.port = new_name; }
                            } else {
                                if old_name.contains("Access Key") { app.s3_server.access_key = new_name; }
                                else if old_name.contains("Secret Key") { app.s3_server.secret_key = new_name; }
                                else if old_name.contains("Port") { app.s3_server.port = new_name; }
                            }
        ...
        PopupState::ConfirmDelete { name } => {
            match key.code {
                ...
                KeyCode::Enter | KeyCode::Char('y') | KeyCode::Char('Y') => {
                    ...
                    let pane = if is_left { &mut app.left_pane } else { &mut app.right_pane };
        ...
        PopupState::SwitchAccountMenu { selected_idx } => {
            match key.code {
                KeyCode::Esc => app.popup_state = PopupState::None,
                KeyCode::Up => {
                    let next_idx = if selected_idx == 0 { app.accounts.len() - 1 } else { selected_idx - 1 };
        ...
        match key.code {
            ...
            KeyCode::Char('y') | KeyCode::Char('Y') => {
                ...
                if !pane.items.is_empty() {
                    let name = pane.items[pane.selected_idx].name.clone();
                    if name != ".." {
                        app.popup_state = PopupState::RenameInput { old_name: name.clone(), buffer: name };
                    }
                }
            }
            ...
            KeyCode::Char('s') | KeyCode::Char('S') => {
                ...
                app.popup_state = PopupState::SwitchAccountMenu { selected_idx: app.active_account_idx };
            }
            _ => {}
        }
    }
...
async fn trigger_special_action(app: &mut App, selected_idx: usize) {
    ...
    let full_path = Path::new(&pane.path).join(&selected_item_name).to_string_lossy().to_string();
        ...
        1 => {
            ...
            match Operations::favorite(&app.active_account, &full_path).await {
                ...
                Err(e) => {
                    ...
                    if let Ok(_) = Operations::unfavorite(&app.active_account, &full_path).await {
                        app.popup_state = PopupState::Message {
                            title: "Yêu thích".to_string(),
            ...
            match Operations::list_trash(&app.active_account).await {
                Ok(trash_items) => {
                    if trash_items.is_empty() {
                        ...
                        let lines = trash_items.iter().enumerate().map(|(i, item)| {
                            let type_prefix = if item.is_dir { "[DIR]" } else { "[FILE]" };
                            format!("({}) {} - {} - {}", i + 1, type_prefix, item.name, item.mod_time)
                        }).collect();
                        app.popup_state = PopupState::ViewFile {
        ...
        4 => {
            ...
            let res = if pane.is_local {
                match std::fs::metadata(&full_path) {
                    Ok(meta) => {
                        let s = format!("File: {}\nSize: {} bytes\nType: {}\nModified: {:?}", selected_item_name, meta.len(), if meta.is_dir() { "directory" } else { "file" }, meta.modified());
                        Ok(s)
                    }
...
pub async fn trigger_sync(app: &mut App) {
    app.is_loading = true;
    let local_path = if app.left_pane.is_local { &app.left_pane.path } else { &app.right_pane.path };
    let remote_path = if !app.left_pane.is_local { &app.left_pane.path } else { &app.right_pane.path };
