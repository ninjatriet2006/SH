...
pub fn distribute_files(files: &[PathBuf], settings: &Settings) {
    ...
    let (num_folders, files_per_folder) = if choice.starts_with("3") {
        ...
        if total_final % per_folder != 0 { n += 1; }
        (n, per_folder)
    } else {
        // Balanced mode
        let mut n = 1;
        while ((total_final + n - 1) / n) > settings.max_files_per_folder {
            n += 1;
        }
        let per_folder = (total_final + n - 1) / n;
        (n, per_folder)
    };

    if is_oneshot && num_folders > 1 {
        println!("Cảnh báo: Số lượng file ({}) vượt mức 1 thư mục Oneshot. Sẽ chia thành {} thư mục Oneshot. Part", total_final, num_folders);
    }
            ...
            for entry in entries.flatten() {
                if entry.path().is_dir() {
                    ...
                    if let Some(caps) = re.captures(&dir_name) {
                        if let Ok(y) = caps[1].parse::<usize>() {
                            if y > max_y {
                                max_y = y;
                            }
                        }
