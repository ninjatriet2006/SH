...
impl ApiClient {
    ...
    pub async fn test_api(&self, base_url: &str, api_key: &str) -> ApiStatus {
        ...
        if status.as_u16() == 402 {
            ...
            let msg = if let Ok(err_resp) = parsed {
                err_resp.error.map(|e| e.message).unwrap_or_else(|| "Insufficient credits".to_string())
            } else {
        ...
        if status.as_u16() == 401 || status.as_u16() == 403 {
            let parsed: Result<ErrorResponse, _> = serde_json::from_str(&body_text);
            let msg = if let Ok(err_resp) = parsed {
                err_resp.error.map(|e| e.message).unwrap_or_else(|| "Unauthorized".to_string())
            } else {
        ...
        if status.as_u16() == 404 && body_text.contains("Use /v1/<provider>/<path>") && !base_url.contains("opengateway.gitlawb.com") {
            // Thử gọi lại với /openai/models
            let clean_base = base_url.trim_end_matches('/');
            let retry_url = format!("{}/openai/models", clean_base);
            
            let retry_resp = self.client.get(&retry_url)
                .header("Authorization", format!("Bearer {}", api_key))
                .send()
                .await;
    ...
    pub async fn fetch_models(&self, base_url: &str, api_key: &str) -> Result<Vec<String>, String> {
        ...
        let status = response.status();
        let body_text = response.text().await.map_err(|e| format!("Không thể đọc body: {}", e))?;
        ...
        if !status.is_success() {
            ...
            if status.as_u16() == 404 && body_text.contains("Use /v1/<provider>/<path>") && !base_url.contains("opengateway.gitlawb.com") {
                let clean_base = base_url.trim_end_matches('/');
                let retry_url = format!("{}/openai/models", clean_base);
                let retry_response = self.client.get(&retry_url)
                    .header("Authorization", format!("Bearer {}", api_key))
                    .send()
                    .await
                ...
                if retry_response.status().is_success() {
                    let retry_body = retry_response.text().await.map_err(|e| format!("Không thể đọc body: {}", e))?;
                    let res: OpenAIModelsResponse = serde_json::from_str(&retry_body)
            ...
            let parsed: Result<ErrorResponse, _> = serde_json::from_str(&body_text);
            let msg = if let Ok(err_resp) = parsed {
                err_resp.error.map(|e| e.message).unwrap_or_else(|| format!("HTTP {}", status))
            } else {
                format!("HTTP {}", status)
            };
            return Err(msg);
        ...
        let res: OpenAIModelsResponse = serde_json::from_str(&body_text)
            .map_err(|e| format!("Lỗi parse JSON: {}", e))?;
