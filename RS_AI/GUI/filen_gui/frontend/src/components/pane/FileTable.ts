...
const ALL_COLUMNS: { key: SortKey; label: string; resize?: PaneColKey }[] = [
  ...
  { key: 'permissions', label: 'Permissions', resize: 'permissions' },
  { key: 'owner', label: 'Owner', resize: 'owner' },
  { key: 'group', label: 'Group', resize: 'group' },
];
...
export class FileTable {
  ...
  constructor(opts: FileTableOptions) {
    ...
    this.observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          if (index >= 0 && this.opts.pane === 'left') {
            const rec = this.rows[index];
            if (!rec.thumbnailLoaded) {
    ...
    opts.files.forEach((f, i) => {
      ...
      row.addEventListener('dblclick', async (e) => {
        ...
        if (f.is_dir) {
          ...
          if (opts.pane === 'left' && !rec.path.startsWith('trash://')) {
            ...
            alert('File này đang ở trên Cloud hoặc Thùng rác. Bạn cần tải về Local để mở!');
          }
        }
      });
      ...
      if (this.opts.pane === 'left' && this.isImageFile(f)) {
