...
# =================================================================
# SCRIPT XỬ LÝ FILE THEO QUY TẮC N (BẢN TỐI ƯU ĐA LUỒNG & AUTO-INDEX)
# =================================================================
...
if [ "$total_webp" -gt 0 ]; then
    ...
    echo "   -> Đang xử lý đa luồng ($threads cores)..."
...
echo -e "3. Tổng cộng: $TOTAL_FINAL file ảnh.\n"

# --- BƯỚC 6: Chia folder (AUTO-INDEX) ---
read -p "4. Nhập số Chương (X): " chapter_x

# Thuật toán quét Index hiện tại:
# Tìm tất cả folder "Chapter X.*", lấy số sau dấu chấm, sắp xếp số và lấy số lớn nhất.
last_sub_idx=$(ls -d "Chapter ${chapter_x}."* 2>/dev/null | sed "s/Chapter ${chapter_x}\.//" | sort -n | tail -1)

if [ -z "$last_sub_idx" ]; then
    curr_sub_idx=1
    echo "   -> Không tìm thấy folder cũ. Bắt đầu từ Chapter ${chapter_x}.1"
else
    curr_sub_idx=$((last_sub_idx + 1))
    echo "   -> Đã tìm thấy Chapter ${chapter_x}.${last_sub_idx}. Tiếp tục từ Chapter ${chapter_x}.${curr_sub_idx}"
fi

# Tính toán số lượng folder tối ưu (Quy tắc N)
n=1
